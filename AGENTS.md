...
A reusable pnpm monorepo template for pixel-perfect UI quality pipelines. Design screens
in Storybook, generate golden PNG screenshots, then validate your production app matches.

## Architecture

Three pipeline apps plus shared design tokens:
...
| E2E | `apps/e2e/` | Playwright visual regression against your production app |
| Web | `apps/web/` | Placeholder production app (replace with your own) |
...
cross-ui-template/
  ...
  scripts/init.sh            # Template personalization (bash)
...
| Script | Description |
|--------|-------------|
| `pnpm run init` | Personalize template (replace @cross-ui/ scope) |
| `pnpm run test` | Full sanity chain: format, lint, type-check |
| `pnpm run spec:dev` | Storybook dev server (port 6006) |
| `pnpm run spec:build` | Build static Storybook |
| `pnpm run app:dev` | Start web app on port 8080 (placeholder) |
| `pnpm run app:build` | Build web app |
| `pnpm run artifacts:capture` | Capture golden PNGs from Storybook |
| `pnpm run e2e:web` | E2E visual regression (web) |
| `pnpm run e2e:web:pw` | E2E visual regression via Playwright directly (no wrapper) |
| `pnpm run pipeline:capture` | Capture golden PNGs |
| `pnpm run pipeline:validate` | Run E2E visual regression |
| `pnpm run pipeline:full` | Full pipeline: capture + validate |
| `pnpm run clean` | Remove all build artifacts recursively |
| `pnpm run bumpDependencies` | Bump deps in all apps (each has its own `.ncurc.json` doctor) |
| `pnpm run release` | Release: bump all package.json versions, generate changelog, create git tag |
...
Run `pnpm run init` after cloning to replace the `@cross-ui/` placeholder scope and
project name with your own values.
...
AI agent skills live in `.claude/skills/` and provide coding conventions for this template.
...
They define the visual contract that your production app must replicate pixel-perfectly.
...
### Your Framework (add when ready)

Path: `.claude/skills/your-framework/SKILL.md`

When you choose your production UI framework, create a skill with patterns and conventions.
Examples: `nextjs-app-router`, `sveltekit-pages`, `dioxus-tailwind`, `flutter-widgets`.
